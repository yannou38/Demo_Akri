...
async def main():
    ...
    server = Server()
    await server.init()
    server.set_endpoint('opc.tcp://192.168.236.3:4840/opcua/')
    ...
    var_temperature = await obj_vplc.add_variable(idx, 'temperature', 0.1)
    var_pressure = await obj_vplc.add_variable(idx, 'pressure', 0.1)
    var_pumpsetting = await obj_vplc.add_variable(idx, 'pumpsetting', 'test')
    var_pumpstatus = await obj_vplc.add_variable(idx, 'pumpstatus', 'test')
    var_timestamp = await obj_vplc.add_variable(idx, 'timestamp', 'test')

    # Read Sensor Data from Kaggle
    df = pd.read_csv("sensor.csv")
    # Only use sensor data from 03 and 01 (preference)
    sensor_data = pd.concat([df["sensor_03"], df["sensor_01"], df['machine_status'], df['timestamp']], axis=1)
    ...
    async with server:
        ...
        while True:
            for row in sensor_data.itertuples():
                ...
                await var_pumpstatus.write_value(str(row[3]))
                await var_timestamp.write_value(str(row[4]))
